...
class TestSearchAll:
    ...
    def test_search_without_query_should_have_correct_sorting(
        ...
        assert len(result) > 0
        debug_values = {"authority": [], "previous_dataset": [], "datatype": []}
    ...
    def test_trailing_white_space_should_not_affect_result(
        ...
        no_white_space_result = post(
            url=service_url + "/search", json=body_no_white_space
        ).json()
        white_space_result = post(
            url=service_url + "/search", json=body_white_space
        ).json()
    ...
    def test_special_chars_should_not_affect_result_length(
        ...
        expected = post(url=service_url + "/search", json=body).json()["page"][
            "totalElements"
        ]
    ...
    def test_search_all_total_should_be_sum_of_indices(
        ...
        total_services = post(service_url + "/dataservices").json()["page"][
            "totalElements"
        ]
        total_concepts = post(service_url + "/concepts").json()["page"]["totalElements"]
        total_models = post(service_url + "/informationmodels").json()["page"][
            "totalElements"
        ]
