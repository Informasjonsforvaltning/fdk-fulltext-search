...
@pytest.mark.unit
def test_fetch_data_services_should_initiate_reindex(mock_env, mock_ingest_from_harvester, mock_get, mock_single_reindex):
...
@pytest.mark.unit
def test_fetch_data_services_should_not_initiate_reindex(mock_env, mock_get, mock_ingest_from_harvester, mock_single_reindex):
    fetch_data_services()
    assert mock_ingest_from_harvester.call_count == 1
    ingest_calls = mock_ingest_from_harvester.call_args_list[0][0]
    assert ingest_calls[1] == 'dataservices'
