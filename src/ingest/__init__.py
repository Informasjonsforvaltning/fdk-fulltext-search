...
def fetch_all_content():
    ...
    total_concepts = 0
    if isinstance(concept_status, list):
        for iteration in concept_status:
            total_concepts = total_concepts + iteration["count"]
    else:
        total_concepts += concept_status["count"]
        breakpoint()
    totalElements = info_status["count"] + total_concepts + service_status["count"] + datasets_status["count"]
    result = {
        "took": total_time,
        "total": totalElements,
...
def fetch_information_models():
    ...
    try:
        ...
        result = error_msg(f"fetch informationmodels from {info_url}", err.strerror)
...
def fetch_concepts():
    ...
    try:
        ...
        result = error_msg(f"fetch concepts from {concept_url}", err.strerror)
        logging.error(result["message"])
        return result
...
def fetch_datasets():
    ...
    try:
        ...
        result = error_msg(f"fetch datasets from {dataset_url}", err.strerror)
        logging.error(result["message"])
        return result
...
def fetch_dataservices():
    ...
    try:
        ...
        result = error_msg(f"fetch sataservices from {data_service_url}", err.strerror)
        logging.error(result["message"])
        return result
