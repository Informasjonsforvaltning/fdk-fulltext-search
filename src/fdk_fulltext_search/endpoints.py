...
class SearchPublicServices(Resource):
    def post(self: Any) -> Dict:
        ...
        if len(request.data) == 0:
            ...
            if "page" in body:
                page = body["page"]
            result = client.search_in_index(
                index=IndicesKey.PUBLIC_SERVICES, request=body
            )
        if "error" in result.keys():
            return result
        else:
        ...
        if index_name:
            if index_name not in [
                ...
                abort(
                    ...
                    description={
                        "bad request": "indices '{0}' is not a valid index. Valid indices values are ["
    ...
    def post(self: Any) -> Response:
        ...
        if index_name:
            if index_name not in [
                ...
                abort(
                    http_status_code=400,
                    description="bad request: indices {0} does not exist".format(
                        index_name
                    ),
                )
        else:
